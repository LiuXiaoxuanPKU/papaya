...
parser.add_argument('--world-size', '--ws', default=1, type=int,
                    metavar='WS', help='distributed world size (default: 1)')
...
def main_worker(args):
    ...
    if args.rank is not None: 
        assert type(args.rank) == int, "distributed rank not correctly set"
        args.gpu = args.rank
    ...
    if args.get_macs:
        ...
        input = torch.randn(1, 3, input_size, input_size).cuda(args.gpu)
    ...
    with UtilizationContext(args.get_util) as context:
        ...
        for (i, (images, target)) in enumerate(train_loader):
            ...
            if args.get_speed:
                ...
                if train_step_ct >= 18:
                    if not args.distributed or args.rank == 0:
                        train_ips = np.median(train_ips_list)*args.world_size
