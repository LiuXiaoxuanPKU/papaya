...
def network_to_command(network, bz):
    ...
    --max-tokens %d --update-freq 16 \
    --fp16 \
    --max-update 50000 \
    --required-batch-size-multiple 1 """ % (network, token_per_sample, bz * token_per_sample)
    return cmd


def run_benchmark(network, batch_size, alg, get_mem = False):
    cmd = network_to_command(network, batch_size)
    ...
    for net in networks:
        for alg in algs:
            ...
            for batch_size in range(1, 800, 1):
                ret_code = run_benchmark(net, batch_size, alg)
                if ret_code != 0:
                    try_cnt += 1
                    if try_cnt == 3:
                        break